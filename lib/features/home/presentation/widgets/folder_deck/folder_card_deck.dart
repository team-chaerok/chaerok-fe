...
/// 폴더 탭 카드 스택 — [deckOrder]의 맨 뒤 원소가 "열린" 카드이고, 나머지는
/// 위에 탭만 겹쳐 쌓인다. 겹친 탭을 누르면 [onOpen]으로 전환을 요청하며,
/// 부모가 그 탭과 직전에 열려 있던 탭의 덱 순서를 스위치하면 두 카드가
/// 앞뒤로 자리를 맞바꾸는 전환 애니메이션이 재생된다(값은 프로토타입 확정치).
///
/// 카드 자체의 시각 요소는 [cardBuilder]에 위임하므로, 탭 식별자 타입 [T]가
/// 다른 화면(지역별 홈의 `RegionCode`, 충남 홈의 `HomeCardTab`)끼리 이 스택
/// 메커니즘 하나를 공유한다. 이 파일은 원래 `RegionFilmDeck`에 있던 애니메이션
/// 상수·좌표 계산을 값 변경 없이 그대로 옮긴 것이다.
...
class _FolderCardDeckState<T> extends State<FolderCardDeck<T>>
  ...
  @override
  void didUpdateWidget(FolderCardDeck<T> oldWidget) {
        ...
        return AnimatedBuilder(
          ...
          builder: (context, _) {
            ...
            return Stack(
              ...
              children: [
                for (final tab in widget.deckOrder)
                  _card(
                    tab: tab,
                    peek: peek,
  ...
  Widget _card({
    ...
    final open = tab == widget.deckOrder.last;
    final targetSlot = widget.deckOrder.indexOf(tab);
    ...
    if (swapping) {
      final fromSlot = _fromOrder.indexOf(tab);
      if (tab == _incoming) {
    ...
    return Positioned(
      ...
      child: Transform(
        ...
        child: GestureDetector(
          onTap: open || locked ? null : () => widget.onOpen(tab),
          child: widget.cardBuilder(context, tab, open),
        ),
      ),
    );
  }
