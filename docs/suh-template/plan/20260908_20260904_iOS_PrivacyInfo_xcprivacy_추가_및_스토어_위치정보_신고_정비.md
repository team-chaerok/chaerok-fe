...
- **P0-1** `ios/Runner/PrivacyInfo.xcprivacy` 생성
  - `NSPrivacyCollectedDataTypes` → **Precise Location**(`NSPrivacyCollectedDataTypeLocationPreciseLocation`)
    - 목적: **App Functionality**(`...ForAppFunctionality`) — 지역 판별, 주변 채록 장소·필름롤·날씨 제공
...
- **P2-1** 촬영 사진(`camera` 플러그인)의 EXIF GPS 포함 여부 점검 — 포함 시 S3 업로드 경로로 좌표가 간접 전송될 수 있으므로 데이터 흐름 문서에 각주
- **P2-2** `geolocator` / `permission_handler` / `camera` 등 위치·민감 플러그인의 번들 내 `PrivacyInfo.xcprivacy` 존재를 빌드 산출물에서 확인(플러그인 책임이지만 릴리스 점검 항목으로)
- **P2-3** 개인정보처리방침 웹 문서 별도 이슈 등록
...
| **채록 자체 백엔드** (사진 업로드) | [film_rolls_api.dart:77-88](../../../lib/data/remote/film_rolls_api.dart#L77-L88), [photo_upload_url_request.dart](../../../lib/data/models/photo_upload_url_request.dart) | `sequence`, `contentType`, `contentLength`, `takenAt`만. 사진 바이너리는 S3 presigned PUT | 좌표 메타데이터 미전송 (EXIF는 P2-1에서 점검) |
...
| 방문 인증 사진 좌표 저장 | [photos_table.dart](../../../lib/core/database/tables/photos_table.dart) — **로컬 Drift DB에만** 저장(`latitude`, `longitude` nullable). 서버 동기화 payload에는 미포함 |

### 4.4 결론

앱이 **원시 정밀 좌표를 기기 밖으로 내보내는 경로는 카카오 로컬 API 1곳**이며, 기상청은 좌표에서 파생된 격자값을 받는다. **채록 자체 백엔드는 어떤 경로로도 원시 좌표를 수신·저장하지 않는다.**
...
| # | 결정 | 선택 | 이유 |
| --- | --- | --- | --- |
| D1 | 정밀 위치의 "사용자 신원 연결(linked to identity)" 값 | **Not Linked (`false`)** | §4 코드 분석상 자체 백엔드가 원시 좌표를 계정과 함께 저장하지 않음. 카카오/기상청 전송분도 계정 식별자와 함께 보내지 않음(카카오는 REST 키 헤더만, 기상청은 serviceKey만). 백엔드 재확인은 릴리스 체크리스트 항목으로만 유지 |
...
### 남은 확인 항목 (blocking 아님)

- **BE 재확인**: 위치 인증·방문 인증 로그·분석 파이프라인에서 좌표를 별도 수집하지 않는지 (D1 근거 보강용, 릴리스 체크리스트 항목)
- **EXIF 점검** (P2-1): `camera` 플러그인 촬영본에 GPS EXIF가 실리는지 → 실리면 S3 업로드로 좌표가 간접 전송되므로 D2 표기 대상에 자체 백엔드 추가 및 데이터 흐름 문서 갱신
...
채택 안 함(사용자 확정). 범위가 커지고 법무 검토가 필요. 별도 이슈로 분리(P2-3).
...
| S7 | (P2) `camera` EXIF GPS 점검 | 촬영 샘플 EXIF 확인 → 결과를 S1 문서에 반영, 필요 시 S2·S3 갱신 |
...
| EXIF GPS가 사진에 실려 S3로 전송 중이었음(P2-1 미점검 시) | "자체 백엔드 좌표 미전송" 서술이 사실과 불일치 | S7을 P2가 아닌 P1로 승격 검토. 최소한 릴리스 전 1회 점검 필수 |
...
- [ ] EXIF GPS 점검(P2-1) 결과가 데이터 흐름 문서에 반영됨
